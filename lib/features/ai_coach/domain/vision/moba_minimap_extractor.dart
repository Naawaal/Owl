...
class MobaMinimapExtractor {
  ...
  MobaTacticalSnapshot processTokens({
    ...
    final objectives = objectiveOverrides ?? _computeObjectiveTimers(matchTimeSeconds, allies, visibleEnemies);
    for (final obj in objectives) {
      if (obj.isContested) {
        alerts.add('${obj.name} is actively contested!');
      } else if (obj.status == 'upcoming' && obj.timeUntilSpawnSeconds <= 15) {
        alerts.add('${obj.name} spawning in ${obj.timeUntilSpawnSeconds}s - Prepare positioning');
      }
    }
  ...
  List<NeutralObjectiveState> _computeObjectiveTimers(
    ...
    if (matchTimeSeconds < 120) {
      ...
      // Turtle is alive or contested
      final isContested = _isObjectivePitContested(allies, enemies, 0.20, 0.12);
      list.add(NeutralObjectiveState(
        name: 'Turtle',
    ...
    if (matchTimeSeconds >= 420 && matchTimeSeconds < 480) {
      ...
      final isContested = _isObjectivePitContested(allies, enemies, 0.80, 0.88);
      list.add(NeutralObjectiveState(
        name: 'Lord',
        status: isContested ? 'contested' : 'alive',
        isContested: isContested,
